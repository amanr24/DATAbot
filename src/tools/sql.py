    ...
    try:
        ...
        for schema_name in inspector.get_schema_names():
            ...
            for table_name in tables:
                table_info = {"table_name": table_name, "columns": [], "primary_key": [], "foreign_keys": [], "indexes": []}
                ...
                for col in columns:
                    ...
                    query = build_query(col_name_quoted, table_name_quoted, n_samples, metadata["dialect"])
                    ...
                    table_info["columns"].append({"name": col_name, "type": col_type, "sample_values": samples})

                pk_constraint = inspector.get_pk_constraint(table_name, schema=schema_name)
                table_info["primary_key"] = pk_constraint.get("constrained_columns", [])

                fks = inspector.get_foreign_keys(table_name, schema=schema_name)
                table_info["foreign_keys"] = [{"local_cols": fk["constrained_columns"], "referred_table": fk["referred_table"], "referred_cols": fk["referred_columns"]} for fk in fks]
